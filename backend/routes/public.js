...
router.post("/trial", async (req, res, next) => {
  try {
    ...
    if (!company || !subdomain || !email) {
      return res
        .status(400)
        .json({ error: "company, subdomain, email required" });
    }

    subdomain = normalizeSubdomain(subdomain);
    if (!subdomain)
      return res.status(400).json({ error: "Некорректный поддомен" });
    ...
    res.json({
      ...
      adminEmail: email,
      adminPassword: password, // показываем сразу (в реале лучше отправлять на почту)
    });
  } catch (e) {
    if (e && e.code === 11000) {
