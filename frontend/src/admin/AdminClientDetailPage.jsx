...
const CLIENT_PATHS = [(id)=>`/api/users/admin/${id}`,(id)=>`/api/admin/users/${id}`,(id)=>`/api/clients/admin/${id}`];
const ORDERS_BY_USER_PATHS = [(id)=>`/api/orders?user=${id}`,(id)=>`/api/orders/admin?user=${id}`,(id)=>`/api/admin/orders?user=${id}`];

const firstDefined=(...v)=>v.find(x=>x!=null);
const joinUrl=(base,path)=>path?.startsWith("http")?path:`${base}${path?.startsWith("/")?"":"/"}${path||""}`;

function normalizeClient(raw){
  const c=firstDefined(raw?.client,raw?.user,raw)||{};
  return { id:c._id||c.id, firstName:c.firstName||c.name||"", lastName:c.lastName||"", phone:c.phone||"", email:c.email||"", createdAt:c.createdAt||null, rating:typeof c.rating==="number"?c.rating:null };
}
function normalizeOrders(raw){
  const arr=firstDefined(raw?.orders,raw?.items,raw?.data,Array.isArray(raw)?raw:[])||[];
  return arr.map(o=>({ id:o._id||o.id, createdAt:o.createdAt||o.date||null, totalPrice:Number(o.totalPrice||0), items:Array.isArray(o.items)?o.items:[], address:o.novaPoshta||o.address||"" }));
}
  ...
  if (loading){
    return (
      <div className="client-detail-page admin-content with-submenu">
        ...
        <div className="client-detail-content"><div className="loader-standalone"><span className="loader"/></div></div>
      </div>
    );
  }
  if(!client){
      ...
      <div className="client-detail-content">
        ...
        <div className="clients-topbar">
          ...
          <div className="clients-topbar-right">
            <button className="btn-outline" onClick={()=>alert("Создать заказ")}>Создать заказ</button>
            <button className="btn-danger" onClick={()=>alert("Удалить клиента")}>Удалить клиента</button>
          </div>
        </div>
        ...
        <div className="card client-orders-card">
          ...
          {ordersLoading ? (
            ...
            const sum=o.totalPrice||items.reduce((s,it)=>s+(Number(it.product?.price)||0)*(Number(it.quantity)||1),0);
                ...
                <div className="order-items">
                  {items.slice(0,isOpen?undefined:1).map((it,idx)=>(
                    <div key={idx} className="order-item">
                      <img
                        alt="Фото"
                        src={(it.product?.images?.[0] && joinUrl(BASE_URL,it.product.images[0])) ||
