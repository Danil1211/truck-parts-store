...
/* helpers */
const sortByDate = (arr) => [...arr].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
...
const makeBlobUrl = (blob) => { const u = URL.createObjectURL(blob); tmpObjectUrls.add(u); return u; };
const revokeTmpUrl = (u) => { if (u && /^blob:/i.test(u) && tmpObjectUrls.has(u)) { URL.revokeObjectURL(u); tmpObjectUrls.delete(u); } };
...
function VoiceMessage({ audioUrl, initialDuration = 0 }) {
  ...
  const ensureFiniteDuration = () => {
    ...
    if (!Number.isFinite(a.duration) || a.duration === 0) {
      if (!seekingFixApplied.current) {
        ...
        const onSeeked = () => { if (Number.isFinite(a.duration) && a.duration > 0) setDuration(a.duration); a.currentTime = 0; a.removeEventListener("seeked", onSeeked); };
  ...
  useEffect(() => {
    const a = audioRef.current;
    if (!a) return;
    const onLoaded = () => { Number.isFinite(a.duration) && a.duration > 0 ? setDuration(a.duration) : ensureFiniteDuration(); };
  ...
  const seekToClientX = (clientX) => {
    const el = barRef.current, a = audioRef.current;
    if (!el || !a || !duration) return;
    const rect = el.getBoundingClientRect();
    const x = Math.min(Math.max(0, clientX - rect.left), rect.width);
    const next = (x / rect.width) * duration;
    a.currentTime = next; setCurrent(next);
  };

  const onPointerDown = (e) => {
    e.preventDefault(); setDragging(true); seekToClientX(e.clientX);
  ...
  return (
    <div className="voice-bubble">
      <button className={`icon-btn chat-voice-btn ${playing ? "is-playing" : "is-paused"}`} onClick={toggle} title={playing ? "Пауза" : "Воспроизвести"}>
        {playing ? "⏸" : "▶️"}
      </button>

      <div className="voice-bar" ref={barRef} onPointerDown={onPointerDown} role="slider"
           aria-valuemin={0} aria-valuemax={Math.max(0, Math.round(duration || 0))}
           aria-valuenow={Math.max(0, Math.round(current || 0))} tabIndex={0}
           onKeyDown={(e)=>{ const a=audioRef.current; if(!a) return; if(e.key==="ArrowLeft") a.currentTime=Math.max(0,a.currentTime-5); if(e.key==="ArrowRight") a.currentTime=Math.min(duration||0,a.currentTime+5); }}>
      ...
      <audio ref={audioRef} src={audioUrl} preload="metadata" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} style={{ display: "none" }} />
    </div>
  );
}
...
function AudioPreview({ blob, seconds = 0, onRemove }) {
  ...
  const [playing, setPlaying] = useState(false);
  const audioRef = useRef(null);
  useEffect(() => { if (!blob) return; const u = makeBlobUrl(blob); setUrl(u); return () => revokeTmpUrl(u); }, [blob]);
  useEffect(() => {
  ...
  return (
    <div className="audio-preview">
      <button className="audio-preview__btn" onClick={() => { const a = audioRef.current; if (!a) return; playing ? a.pause() : (a.currentTime = 0, a.play()); }} title={playing ? "Пауза" : "Прослушать"}>{playing ? "⏸" : "▶️"}</button>
...
export default function AdminChatPage() {
  ...
  const loadChats = async () => {
    ...
    try {
      ...
      if (firstChatsLoadRef.current) { setLoadingChats(false); firstChatsLoadRef.current = false; }
    }
  };

  useEffect(() => { loadChats(); const iv = setInterval(loadChats, 4000); return () => clearInterval(iv); }, []); // eslint-disable-line
  useEffect(() => {
    ...
    if (found) { pendingOpenId.current = null; handleSelectChat(found); }
  }, [chats]);
  ...
  useEffect(() => {
    if (!selected) return;
    firstThreadLoadRef.current = true; setLoadingThread(true);
    const load = async () => {
      await loadMessages();
      if (firstThreadLoadRef.current) { setLoadingThread(false); firstThreadLoadRef.current = false; }
      try { const info = await api.get(`/api/chat/admin/user/${selected.userId}`, { params: { _: Date.now() } }); setSelectedUserInfo(normalizeUserInfo(info)); }
      catch { } finally { if (loadingInfo) setLoadingInfo(false); }
    };
    load();
  ...
  useEffect(() => { setActiveChatId(selected?.userId || null); return () => setActiveChatId(null); }, [selected, setActiveChatId]);

  const openChatById = async (userId) => {
    if (!userId) return;
    const chat = chats.find((c) => String(c.userId) === String(userId));
    if (chat) handleSelectChat(chat);
    else { pendingOpenId.current = String(userId); await loadChats(); }
  };

  useEffect(() => {
    const stored = sessionStorage.getItem("openChatId");
    if (stored) { sessionStorage.removeItem("openChatId"); openChatById(stored); }
  }, []);

  useEffect(() => {
    const onOpenInPage = (e) => { const id = e.detail?.chatId; if (id) openChatById(id); };
  ...
  const handleSelectChat = async (c) => {
    ...
    setFiles([]); setInput(""); setShowQuick(false); setShowEmoji(false);
    setIsAutoScroll(true); setAudioPreview(null); setRecording(false); setRecordingTime(0);
    clearInterval(recordingTimer.current); setLoadingInfo(true); resetUnread(c.userId);
    try { const info = await api.get(`/api/chat/admin/user/${c.userId}`, { params: { _: Date.now() } }); setSelectedUserInfo(normalizeUserInfo(info)); }
  ...
  const handleDeleteChat = async (chat) => {
    ...
    if (selected?.userId === uid) { setSelected(null); setMessages([]); setSelectedUserInfo(null); setActiveChatId(null); }
    await loadChats();
  };
        ...
        c.userId === userId
          ? {
              ...
              lastMessageObj: {
                ...(c.lastMessageObj || {}),
                fromAdmin: true,
                text: payload.text || (payload.imageUrls?.length ? "📷 Фото" : "") || (payload.audioUrl ? "🎤 Голосовое" : ""),
                read: true,
                createdAt: new Date().toISOString(),
              },
              lastMessage: payload.text || (payload.imageUrls?.length ? "📷 Фото" : "") || (payload.audioUrl ? "🎤 Голосовое" : "—"),
            }
          : c
      )
    );
  ...
  const replaceTmp = (tmpId, real) => {
    ...
    setMessages((prev) => {
      ...
      if (tmp?._local) { (tmp.imageUrls || []).forEach(revokeTmpUrl); revokeTmpUrl(tmp.audioUrl); }
  ...
  const handleAudioSend = async () => {
    ...
    const optimistic = pushOptimistic({ audioUrl: localUrl, tempKind: "audio", _local: true, _durationHint: recordingTime });
    const form = new FormData();
    form.append("audio", audioPreview, "voice.webm");
    files.forEach((f) => form.append("images", f));
    setFiles([]); setAudioPreview(null); setRecordingTime(0);
    try {
      const res = await api.post(`/api/chat/admin/${selected.userId}`, form, { headers: { "Content-Type": "multipart/form-data" } });
      await typingOff();
      const real = pickMessage(res);
      if (real && real._id) replaceTmp(optimistic._id, real);
  ...
  const sendMedia = async ({ audio, images }) => {
    ...
    const optimistic = pushOptimistic({ text: input.trim() || "", imageUrls: localImageUrls, audioUrl: localAudioUrl, tempKind: audio ? "audio" : null, _local: true });
    const form = new FormData();
  ...
  const handleScroll = () => {
    ...
    // текущая дата для поповера
    const seps = Array.from(el.querySelectorAll(".day-sep"));
    let current = null;
    const threshold = 60;
    seps.forEach((s) => { const relTop = s.offsetTop - el.scrollTop; if (relTop <= threshold) current = s; });
  ...
  useEffect(() => {
    const poll = async () => { try { const res = await api.get(`/api/chat/typing/statuses`, { params: { _: Date.now() } }); if (res && typeof res === "object") setTypingMap(res.data || res); } catch {} };
  ...
  const threadItems = useMemo(() => {
    const out = []; let prevDay = "";
    for (const m of messages) {
      const dKey = new Date(m.createdAt || Date.now()).toDateString();
      if (dKey !== prevDay) { prevDay = dKey; out.push({ type: "sep", key: `sep-${dKey}`, label: dayLabel(m.createdAt || Date.now()) }); }
                ...
                return (
                  ...
                  >
                    ...
                    <div className="chat-meta">
                      <div className="chat-title"><span className="chat-name">{c.name}</span></div>
                      <div className="chat-phone">{c.phone}</div>
                      <div className="chat-last">{(c.lastMessage || "").slice(0, 64)}</div>
                    </div>
                    <button className="chat-delete" title="Удалить чат" onClick={(e) => { e.stopPropagation(); handleDeleteChat(c); }}>×</button>
                  </div>
                );
              })
            )}
            ...
            <>
              ...
              <div className="thread" ref={messagesRef} onScroll={handleScroll}>
                {threadItems.map((it) => {
                  ...
                  const m = it.msg;
                  return (
                    <div key={it.key} className={`bubble ${m.fromAdmin ? "out" : "in"}`}>
                      ...
                      {m.audioUrl && <VoiceMessage audioUrl={withApi(m.audioUrl)} initialDuration={m._durationHint || 0} />}
              ...
              <div className="composer" ref={composerRef}>
                ...
                {audioPreview && <AudioPreview blob={audioPreview} seconds={recordingTime} onRemove={() => setAudioPreview(null)} />}
                ...
                <button className={`icon-btn mic ${recording ? "mic--rec" : ""}`} onClick={startOrStopRecording} disabled={!!audioPreview} title={recording ? `Остановить запись (${fmt(recordingTime)})` : "Записать голосовое"}>
                  {Svg.mic}
                  {recording && <span className="mic__badge">{fmt(recordingTime)}</span>}
                </button>

                {!recording ? (
                  <button className="send-btn" onClick={handleSend} title="Отправить">{Svg.send}</button>
                ) : (
              ...
              {showEmoji && (
                <div className="emoji-popover" ref={emojiRef}>
                  <Picker
                    onEmojiClick={(emojiData) => { setInput((v) => v + emojiData.emoji); setShowEmoji(false); }}
                    width={320} height={260}
                    searchDisabled skinTonesDisabled
                    previewConfig={{ showPreview: false }} lazyLoadEmojis
                  />
                </div>
              )}
            </>
              ...
              <>
                ...
                <div className="user-props">
                  ...
                  <div className="user-link-row">
                    <b>Страница:</b>{" "}
                    {withSite(selectedUserInfo.lastPageUrl || "") ? (
