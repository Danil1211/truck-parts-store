...
export default function Register() {
  ...
  async function onSubmit(e) {
    ...
    if (sd.length < 3) return setMsg({ type: "error", text: "Поддомен минимум 3 символа, латиница/цифры/-" });
    ...
    try {
      const r = await fetch(`${API}/api/public/signup`, {
        ...
        body: JSON.stringify({ company: company.trim(), subdomain: sd, email: email.trim(), password }),
      });
      const data = await r.json();
      if (!r.ok) throw new Error(data?.error || "Ошибка регистрации");

      // сохраняем для dev и идём в админку
      localStorage.setItem("tenantId", data.tenantId);
      localStorage.setItem("token", data.token);

      // PROD (когда будет DNS):
      // window.location.href = `https://${data.subdomain}.shopik.com/admin`;
    ...
    <section className="max-w-lg mx-auto">
      ...
      <form onSubmit={onSubmit} className="grid gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-600">.shopik.com</span>
        </div>
        ...
        <button disabled={loading} className="bg-blue-700 text-white rounded p-2 disabled:opacity-60">
